...
int MOAIMoviePlayerAndroid::_init ( lua_State* L ) {
    ...
    if ( movie == NULL ) {
		...
		ZLLogF ( ZLLog::CONSOLE, "MOAIMoviePlayerAndroid: Unable to find java class %s", "com/ziplinegames/moai/MoaiMoviePlayer" );
    }
    else {
    	...
    	if ( init == NULL ) {
			...
			ZLLogF ( ZLLog::CONSOLE, "MOAIMoviePlayerAndroid: Unable to find static java method %s", "init" );
    	}
    	else {
...
int MOAIMoviePlayerAndroid::_play ( lua_State* L ) {
    ...
    if ( movie == NULL ) {
    	...
    	if ( play == NULL ) {
			...
			ZLLogF ( ZLLog::CONSOLE, "MOAIMoviePlayerAndroid: Unable to find static java method %s", "play" );
    	}
    	else {
...
int MOAIMoviePlayerAndroid::_pause ( lua_State* L ) {
    ...
    if ( movie == NULL ) {
    	...
    	if ( pause == NULL ) {
			...
			ZLLogF ( ZLLog::CONSOLE, "MOAIMoviePlayerAndroid: Unable to find static java method %s", "pause" );
    	}
    	else {
...
int MOAIMoviePlayerAndroid::_stop ( lua_State* L ) {
    ...
    if ( movie == NULL ) {
    	...
    	if ( stop == NULL ) {
			...
			ZLLogF ( ZLLog::CONSOLE, "MOAIMoviePlayerAndroid: Unable to find static java method %s", "stop" );
    	}
    	else {
...
//----------------------------------------------------------------//
extern "C" JNIEXPORT void JNICALL Java_com_moaisdk_core_MoaiMoviePlayer_AKUNotifyMoviePlayerCompleted ( JNIEnv* env, jclass obj ) {

	MOAIMoviePlayerAndroid::Get ().NotifyMoviePlayerCompleted ();
}

//----------------------------------------------------------------//
extern "C" JNIEXPORT void JNICALL Java_com_moaisdk_core_MoaiMoviePlayer_AKUNotifyMoviePlayerReady ( JNIEnv* env, jclass obj ) {
