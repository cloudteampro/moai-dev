...
int MOAIFmodStudioChannel::_moveVolume ( lua_State* L ) {
	...
	action->Start ( MOAISim::Get ().GetActionMgr ().GetDefaultParent (), false );
...
bool MOAIFmodStudioChannel::ApplyAttrOp ( u32 attrID, MOAIAttrOp& attrOp, u32 op ) {
	...
	if ( MOAIFmodStudioChannelAttr::Check ( attrID )) {
		...
		if ( attrID == ATTR_PITCH ) {
			this->mPitch = attrOp.Apply ( this->mPitch, op, MOAIAttrOp::ATTR_READ_WRITE, MOAIAttrOp::ATTR_TYPE_FLOAT );
			this->SetPitch ( this->mPitch );
			return true;
		}

		if ( attrID == ATTR_PAN ) {
			this->mPan = attrOp.Apply ( this->mPan, op, MOAIAttrOp::ATTR_READ_WRITE, MOAIAttrOp::ATTR_TYPE_FLOAT );
